...
DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `CreatePerformanceReview`(IN `_criteria` VARCHAR(255), IN `_date_reviewed` DATE, IN `_employee_reviewed` INT(11), IN `_ratings` DOUBLE, IN `_reviewer` INT(11))
BEGIN
...
DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `CreateTicket`(IN `_transac` VARCHAR(255), IN `_employeeId` INT(11), IN `_item` VARCHAR(255), IN `_quantity` INT(11), IN `_description` VARCHAR(255))
BEGIN
...
DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `RetrievePerformanceReviews`()
BEGIN
	SELECT 
    	pr.id as performance_review_id,
		concat(emp_reviewed.firstname,' ', emp_reviewed.lastname) as employee_reviewed,
...
DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `UpdateEmployee`(IN `_employeeId` INT(11), IN `_firstname` VARCHAR(255), IN `_middlename` VARCHAR(255), IN `_lastname` VARCHAR(255), IN `_mobileno` VARCHAR(255), IN `_gender` VARCHAR(255), IN `_email` VARCHAR(255), IN `_birthdate` DATE, IN `_street` VARCHAR(255), IN `_city` VARCHAR(255), IN `_country` VARCHAR(255), IN `_roleId` INT(11))
BEGIN
	SET sql_safe_updates = 0;
    SET @update_id := 0;
...
DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `UpdateTicket`(IN `_ticketId` INT(11), IN `_employeeId` INT(11), IN `_description` VARCHAR(255), IN `_item` VARCHAR(255), IN `_quantity` INT(11), IN `_status` INT(11))
BEGIN
	SET sql_safe_updates = 0;
	SET @update_id := 0;
    IF _description IS NOT NULL then
    UPDATE ticket
    SET description = _description , id = (SELECT @update_id := id)
    WHERE id = _ticketId AND employeeId = _employeeId;
    END IF;
