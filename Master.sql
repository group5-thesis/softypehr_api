SELECT * FROM softype.department_employees;meetingDELIMITER $$
...
BEGIN
        ...
        SET approverId = _approverId, status = 0, resolve_date = date(now()), remarks = "Request Approved", id = (SELECT @update_id := id)
        WHERE id = _office_requestId;
        END IF;	
        IF _indicator = 0 THEN
        UPDATE office_request
        SET approverId = _approverId, status = 0, resolve_date = date(now()), remarks = "Request Rejected", id = (SELECT @update_id := id)
        WHERE id = _office_requestId;
        END IF;
...
BEGIN
        ...
        INSERT INTO office_request(transaction_no, employeeId, item, quantity, approverId, price, total_price, date_needed, purpose)
        VALUES (_transac, _employeeId, _item, _quantity, GetAdmin(), _price, _total_price, _date_needed, _purpose );
...
BEGIN
        SELECT 
            ...
            emp_h.profile_img as department_head_profileImg
            #concat(emp_m.firstname,' ', emp_m.lastname) as department_manager,
...
BEGIN
        ...
        WHERE id = _id AND departmentId = _departmentId;
        END IF;
        UPDATE department_manager
        SET updated_at = date(now()), id = (SELECT @update_id := id)
		WHERE id = _id AND departmentId = _departmentId;
 ...
 --  select employeeId, qr_code  into _employeeID , _qrCode from  softype.user where id = _userID;    
  -- select * , _qrCode as qr_code from  softype.employee  emp where id = _employeeID ;
   ...
   select
       ...
       emp.phil_health_no as phil_health_no ,
       emp.profile_img as profile_img,
       usr.account_type as accountType,
       is_deactivated
   from
softype.employee emp
